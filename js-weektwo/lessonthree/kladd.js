...
We looked at switch statements in Module 1, 
Lesson 3 of Programming Foundations, but we haven't coded one yet.
...
When you have multiple else-if statements, it's time to consider
 a switch statement to make the code more readable and maintainable.
*/

//We begin a switch statement with the value we are checking in brackets.
//  In this case we are checking the value of buttonId.

switch(buttonId) {

}

// We then create a case block for every value we want to check for. 
// We'll add the check for "action" first:
...
/*
If we didn't add the break statements, the code would keep "falling through" to the next case and eventually the default statement, and the code in the default statement would always run,
 meaning arrayToLoopThrough would always be assigned an empty array.
*/
...
/*
This is a better place to store the genre type, as the id shouldn't really be used to store meaningful data, but rather for DOM selection and possibly styling,
 though using classes for styling is considered the better practice.
*/
...
//We can add as many data attributes as we like. 
//Let's add two more to the shooter button, one for pet and one for numberOfGames:
...
const buttonId = event.target.id; // dont work replace it with 
...
To avoid having a very long script.js file, we've moved the code that 
selects the buttons from js/script.js to js/buttons.js and added a link to the 
new .js file in index.html:

<!-- other js files here -->
<script src="js/script.js"></script>
<script src="js/buttons.js"></script>

We need to load js/buttons.js after js/script.js so that we can call the loadGames function
from js/buttons.js. 
We saw in lesson 1 of this module that JavaScript uses hoisting to lift function declarations to the top 
of the code, no matter where they are placed by the developer, but this doesn't apply 
to code loaded in different script files.
*/
...
/*
The selector string is wrapped in square brackets and 
its value is in single quotes because the entire selector string uses double quotes.
*/
...
/*
Inside the loop, we are now adding two click event listeners to every button. 
Every time a button is clicked, the first listener will execute and the loadGames callback function 
will be calledand then the handleActiveClass callback will be called.
*/
...
/*
Just like in the loadGames function, there is an argument that we are calling event. 
This is the event object that gets sent every time the user clicks a button, 
and we can again use event.target to know which button was clicked.
*/
...
// and above we used classList.add("active") to add the active class.
...
/*
Now every time a button is clicked, the active class gets added to it. 
But if we click them all they'll all receive the class and we won't know which one is active.
Every time a button is clicked, we need to remove the active class from all the buttons before
adding the class to the button that was just clicked.
...
/*
Inside the handleActiveClass function, we can loop through the buttons and, 
using the for loops counter variable as an index, 
we can use the classList.remove() to remove the active class from each button.
*/
